...
class Settings(BaseSettings):
    ...
    openai_apps_challenge_token: str | None = Field(
        default=None,
        description=(
            "OpenAI Apps submission-portal domain-verification token. UNSET by "
            "default: while unset, GET /.well-known/openai-apps-challenge MUST "
            "404 (the current live behaviour — this route deploys inert, exactly "
            "like picx_auth_issuer gating OAuth). The token value is generated by "
            "OpenAI's submission portal the first time a human opens the MCP tab "
            "for this plugin — it cannot be fetched or invented here. When "
            "populated, the route returns this exact string as text/plain (raw "
            "token only — no JSON wrapper, no list, no HTML), per OpenAI's "
            "requirement that the challenge endpoint return only that plugin's "
            "verification token. An empty string stays 404 (never serve an "
            "empty/placeholder token — that is worse than 404 as it could be read "
            "as a valid-but-wrong verification attempt)."
        ),
    )
