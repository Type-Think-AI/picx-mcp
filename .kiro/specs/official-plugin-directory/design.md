...
- **Authorization topology — REVISED 2026-09-18: PicX is its own OAuth 2.1 authorization server, and the connector is a pure resource server.** This supersedes the earlier same-day decision in this document to front Google directly with an `OAuthProxy`.
  ...
  That earlier decision was withdrawn because PicX has TWO upstream identity providers, not one: Google directly, and email sign-in through a live Kinde OIDC tenant (`OPENID_PROVIDER_URL`, `/auth/oidc/login`, `/auth/callback/oidc`). Fronting Google alone would leave every email-created account unable to authorize a connector at all.

  **The evidence originally cited here was wrong and is retracted (2026-09-19).** It claimed `hashed_password` on picx-studio's `User` model proved PicX has email/password accounts. It does not: `verify_password` is defined twice and called from nowhere, `app/user/auth.py` declares no routes, and both `create_user` call sites sit in `oauth_manager.py` and pass no password — `hashed_password` is NULL for every user the live path creates. PicX owns no passwords. The conclusion stands on the two-provider fact instead, which is independent and verifiable from configuration.
