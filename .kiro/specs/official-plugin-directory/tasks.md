...
- [ ] 4. Implement token verification, the 401 challenge, and API-key coexistence
  ...
  - [ ] 4.1 Verify the access token on every request before any side effect
    ...
    - **MOSTLY DONE (2026-09-19).** Everything except scope enforcement is now verified and wired:
      - Signature/`iss`/`aud`/`exp` verification and reject-before-side-effect: proven (`tests/test_oauth_401_challenge.py`, Groups A and C) — unchanged from the previous note.
      - **The token→session-key exchange is now wired.** `resolve_api_key()` in `context.py` is async; on a verified access token it reads the JWT's `sub` claim off `AccessToken.subject` (populated by `JWTVerifier.load_access_token`, never re-decoded here) and calls `exchange_token_for_session_key(subject, scopes=...)`, which was already implemented in `auth.py` but never called. All 18 call sites of `get_client()`/`resolve_api_key()` across every tool module are now `await`ed. A bearer token present but unverified (no auth provider configured) is refused with 501, distinct from no-credential-at-all (401).
      - **Added a per-OAuth-token exchange cache** (60s TTL, keyed on the raw JWT string, not the PicX user): `resolve_oauth_grant_key` on picx-studio's side deactivates the user's previous grant key on every exchange, so re-exchanging per tool call would invalidate the key a PRECEDING call in the same multi-tool turn is still using. Verified by mutation (removing the TTL check, dropping scope forwarding, removing the `pxsk_`-first check, and keying the cache on subject instead of the raw token are all caught by `tests/test_context_oauth_exchange.py`, 9 tests).
      - **Still not done, so this box stays unchecked:** *insufficient-scope* rejection is Task 5's scope enforcement, not built yet. This is the only remaining gap for 4.1.
